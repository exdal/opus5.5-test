...
auto RendererInstance::update(this RendererInstance& self, RendererInstanceUpdateInfo& info, const RendererCVar& cvar)
  ...
  if (!info.removed_mesh_bounds.empty()) {
    self.prepared_frame.removed_mesh_bounds_count = static_cast<u32>(info.removed_mesh_bounds.size() / 2);
    auto removed_bounds_buffer = render_context.alloc_transient_buffer(
      vuk::MemoryUsage::eCPUtoGPU,
      info.removed_mesh_bounds.size_bytes()
    );
    std::memcpy(removed_bounds_buffer->mapped_ptr, info.removed_mesh_bounds.data(), info.removed_mesh_bounds.size_bytes());
    self.prepared_frame.removed_mesh_bounds_buffer = std::move(removed_bounds_buffer);
  }
