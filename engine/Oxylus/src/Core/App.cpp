...
auto App::step(this App& self) -> void {
  // An app without the Renderer module (a dedicated server, a tool) has no render context to ask.
  const i32 frame_limit = self.frame_limit > 0 ? self.frame_limit
                          : self.render_context ? self.render_context->context_cvar.cvar_frame_limit.get()
                                                : 0;
  if (frame_limit > 0) {
