package("vulkan-headers")
    ...
    -- oxcity override: github archive downloads are blocked in the sandbox, so the git url is the only one
    -- that works. xmake's git fallback checks out `add_versions("git:<version>")` revisions and otherwise the
    -- raw version string (`1.4.335+0`), which is not a tag in this repo
    add_urls("https://github.com/KhronosGroup/Vulkan-Headers.git", {alias = "git"})
