package("libsdl3")
    ...
    on_install(function (package)
        ...
        local includedirs = {}
        -- oxcity override: forward the link dirs of fetched deps too. SDL's cmake find_library()s every X11 lib,
        -- and when those come from a system/sysroot fetch outside the default search path (the sandbox extracts
        -- them into .sandbox/sysroot) only the headers were ever found
        local linkdirs = {}
        for _, depname in ipairs(packagedeps) do
            ...
            if dep then
                ...
                if depfetch then
                    ...
                    for _, linkdir in ipairs(depfetch.linkdirs or {}) do
        ...
        if #linkdirs > 0 then
            table.insert(configs, "-DCMAKE_LIBRARY_PATH=" .. table.concat(table.unique(linkdirs), ";"))
        end
